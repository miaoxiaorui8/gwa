...
func (casbinService *CasbinService) UpdateCasbinApi(oldPath string, newPath string, oldMethod string, newMethod string) error {
	err := global.GWA_DB.Model(&gormadapter.CasbinRule{}).Where("v1 = ? AND v2 = ?", oldPath, oldMethod).Updates(map[string]interface{}{
		"v1": newPath,
		"v2": newMethod,
	}).Error
	e := casbinService.Casbin()
...
func (casbinService *CasbinService) Casbin() *casbin.CachedEnforcer {
	once.Do(func() {
		a, err := gormadapter.NewAdapterByDB(global.GWA_DB)
		if err != nil {
			global.GWA_Log.Error("数据库连接失败", err)
			return
		}
		text := `
		[request_definition]
		...
		if err != nil {
			global.GWA_Log.Error("字符串加载模型失败", err)
			return
		}
